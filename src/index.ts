...
import { createFreshdeskClient, FreshdeskClient } from "./freshdesk-client.js";
...
let freshdeskClient: FreshdeskClient;
...
server.tool(
  ...
  async (params) => {
    try {
      const result = await getTicket(freshdeskClient, params as { ticket: string });
      return {
        content: [
          {
            type: "text" as const,
...
server.tool(
  ...
  async (params) => {
    try {
      const result = await searchTickets(freshdeskClient, params as {
        query: string;
        status?: string;
        priority?: string;
        limit?: number;
...
server.tool(
  ...
  getAgentTicketsSchema,
  async (params) => {
    try {
      const result = await getAgentTickets(freshdeskClient, params as {
        agent_name: string;
        status?: string;
        limit?: number;
      });
