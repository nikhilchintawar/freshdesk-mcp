...
export class FreshdeskClient {
    ...
    parseTicketId(ticketInput) {
        ...
        throw new Error(`Invalid ticket input: "${ticketInput}". Provide a ticket ID or Freshdesk URL.`);
    }
    /**
     * Get ticket by ID
     */
    async getTicket(ticketId) {
        return this.request(`/tickets/${ticketId}?include=stats`);
    }
    /**
    ...
    async getTimeEntries(ticketId) {
        try {
            return await this.request(`/tickets/${ticketId}/time_entries`);
        }
        catch {
     ...
     */
    async getSatisfactionRating(ticketId) {
        try {
            const ratings = await this.request(`/tickets/${ticketId}/satisfaction_ratings`);
    ...
    async getRequester(requesterId) {
        try {
            return await this.request(`/contacts/${requesterId}`);
        }
        catch {
            return null;
        }
    ...
    async findAgentByName(name) {
        ...
        // Exact match first
        let agent = agents.find((a) => a.contact.name.toLowerCase() === searchName);
        // Partial match if no exact match
        if (!agent) {
            agent = agents.find((a) => a.contact.name.toLowerCase().includes(searchName));
        }
        return agent || null;
    }
    /**
    ...
    async searchTickets(query, options = {}) {
        ...
        const response = await this.request(`/search/tickets?query=${encodedQuery}&page=${page}`);
        return response.results;
    }
    /**
     * Get tickets assigned to a specific agent
     */
    async getAgentTickets(agentId, options = {}) {
        ...
        let tickets = await this.request(`/tickets?${params.toString()}`);
     ...
     */
    async getTicketContext(ticketInput) {
     ...
     */
    async formatTicketSummary(ticket) {
...
export function createFreshdeskClient() {
    ...
    if (!apiKey) {
        throw new Error("FRESHDESK_API_KEY environment variable is required. " +
            "Set it in your MCP server configuration.");
    }
    if (!domain) {
        throw new Error("FRESHDESK_DOMAIN environment variable is required. " +
            "Set it to your Freshdesk subdomain (e.g., 'mycompany' for mycompany.freshdesk.com).");
    }
    return new FreshdeskClient(domain, apiKey);
}
//# sourceMappingURL=freshdesk-client.js.map