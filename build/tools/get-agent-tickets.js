...
export const getAgentTicketsSchema = {
    agent_name: z
        .string()
        .describe("Name of the agent to find tickets for. Supports partial matching (e.g., 'John' will match 'John Doe')."),
    status: z
...
export async function getAgentTickets(client, params) {
    ...
    // Format as summaries
    const tickets = await Promise.all(rawTickets.slice(0, limit).map((t) => client.formatTicketSummary(t)));
    const agentInfo = {
        id: agent.id,
...
function formatAgentTickets(agent, tickets, statusFilter) {
    const lines = [];
...
//# sourceMappingURL=get-agent-tickets.js.map